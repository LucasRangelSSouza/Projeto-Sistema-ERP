...
class Cadastro_de_Produtos():
    ...
    def Botao_Salvar(self):
        if self.Ja_Existe:
            ...
            sql= "INSERT INTO PRODUTOS (ID, CODIGO, REFERENCIA, DESCRICAO, GRUPO, SUBGRUPO, MARCA, DEPARTAMENTO, TRIBUTACAO, UNIDADE, NCM, CEST, PRECO_CUSTO, PRECO_VAREJO, PRECO_ATACADO, LUCRO_VAREJO, LUCRO_ATACADO, ESTOQUE) VALUES (null, '%s', '%s', '%s', %d, %d, %d, %d, %d, %d, '%s', '%s', %d, %d, %d, %d, %d, %d)"   %(Codigo,Referencia,Descricao,Grupo,Subgrupo,Marca,Departamento,Tributacao,Unidade,Ncm,Cest,float((Custo.replace(',','.'))),float((Varejo.replace(',','.'))),float((Atacado,Lucro_Varejo.replace(',','.'))),float((Lucro_Atacado.replace(',','.'))),float((Estoque.replace(',','.'))))
